"use client"
// here we hse client because we are working on a client component 
...
export default function Admin() {
  ...
  const [showPingModal, setShowPingModal] = useState(false); // State variable to track whether Ping Parents modal is visible
  ...
  const fetchStudents = async () => {
    try {
      const response = await fetch('http://localhost:3000/api/fetchstudent'); // Adjust this URL to your API endpoint
...
//
  ...
  return (
    <div className="mx-auto relative">
        ...
        return (
          <div key={student.id} className="flex flex-col sm:flex-row justify-between items-center my-2 border-b py-4">
            <div className="w-full sm:w-1/3 text-left mb-2 sm:mb-0">
              <Link href={`/student/${encodeURIComponent(student.rrn)}`}>
      ...
      {/* Button to open the Ping Parents modal */}
      <button
        onClick={handlePingParents}
        className="px-4 py-2 bg-blue-500 text-white rounded-lg"
      >
        Ping Parents
      </button>
      {/* Modal for Ping Parents */}
      {showPingModal && (
        <div className="modal-backdrop">
          <div className="modal px-4 md:p-[20px] py-12 md:px-8 w-10/12 md:w-1/2">
            <h2 className='text-blue-700 mt-8'>Ping Parents:</h2>

            <div className="flex flex-col md:flex-row justify-between">
              <button
                className="px-4 py-1 bg-red-500 text-white rounded-lg mt-2"
